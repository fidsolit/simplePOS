    ...
    if (match) {
      ...
      try {
        ...
        if (addUser) {
          ...
          setNotification({
            message: "User is not created, Invalid email",
            status: "error",
          });
        }
      } catch (error) {
      ...
      setInputState((oldState) => ({
        ...
        passwordConfirmation: {
          ...
          valid:
            passwordConfirmationInput.validity.valid &&
            inputState.passwordConfirmation.value === inputState.password.value
              ? "valid"
              : "invalid",
        },
      }));
      setNotification({
        message:
          "User is not created, because of you not agree with out terms, Invalid name, email, or password",
        status: "error",
      });
    }
  };
